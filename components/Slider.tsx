...
export default function Slider({
  ...
  return (
    ...
    >
                    ...
                    Array.from({ length: slide.imgAmmount }).map((_, index) => (
                      <SwiperSlide className="relative px-4 xs:px-8 py-12">
                        <div className="w-full h-full flex flex-col justify-center items-center">
                          <div
                            className="group flex flex-col justify-center items-center w-full h-full rounded-t-xl bg-secondary rounded-xl"
                          >
                  ...
                  : (
                    <>
                      ...
                      <SwiperSlide className="relative mb-8">
                        <Link
                          className="group flex flex-col justify-center items-center h-[21rem] rounded-xl bg-secondary gap-y-4"
                          href={`/productos/categorias/${swiperInfo[0].category}`}
                        >
      ...
      <button
        className={`swiper-button-prev absolute bg-black/60 disabled:bg-black/10 rounded-full top-1/2 z-50 ${!showImages ? 'left-3 -translate-y-8' : 'left-6 lg:left-12'}`}
        type="button"
      >
        <BiChevronLeft className="w-5 lg:w-6 h-5 lg:h-6 text-white" />
      </button>
      <button
        className={`swiper-button-next absolute bg-black/60 disabled:bg-black/10 rounded-full top-1/2 z-50 ${!showImages ? 'right-3 -translate-y-8' : 'right-6 lg:right-12'}`}
        type="button"
      >
